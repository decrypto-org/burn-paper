\begin{algorithm}[H]
    \caption{\label{alg.burn-verifier} A contract for verifying burns from the source chain rooted at genesis $\mathcal{G}_s$. This smart contract runs within the target blockchain.}
    \begin{algorithmic}[1]
        \Contract{\textsf{burn-verifier}\text{ extends }\textsf{crosschain}$_{k,m,z}$; \textsf{ERC20}}
            \State\textsf{mapping}(\textsf{address} $\Rightarrow$ \textsf{uint256}) \textsf{balances}
            \State{$\textsf{initialized} \gets \emph{false}$;
                   $\textsf{claimed-events} \gets \emptyset$}
            \Function{\sf initialize}{$\mathcal{G}_s$}
                \If{$\lnot \textsf{initialized}$}
                    \State\textsf{crosschain.initialize}$(\mathcal{G}_s)$
                    \Comment{Initialize with the source chain genesis block}
                    \Let{\textsf{initialized}}{\emph{true}}
                \EndIf
            \EndFunction
            \Function{\sf claim}{$e$}
                \If{$e \in \textsf{claimed-events} \lor \lnot \textsf{event-exists}(e)$}
                    \State\Return{$\bot$}
                \EndIf
                \If{$\lnot \BurnVerify(\{\textsf{targetAddress: msg.sender}\}, e.\textsf{receivingAddress})$}
                    \State\Return{$\bot$}
                \EndIf

                \Let{\textsf{claimed-events}}{\textsf{claimed-events} \cup \{e\}}
                \State{$\textsf{balances[msg.sender]} \pluseq e.\textsf{amount}$}
            \EndFunction
        \EndContract
        \vskip8pt
    \end{algorithmic}
\end{algorithm}
