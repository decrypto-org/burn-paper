\begin{algorithm}[H]
    \caption{\label{alg.burn-verifier} A contract for verifying burns from the source chain. This smart contract runs within the target blockchain.}
    \begin{algorithmic}[1]
        \Contract{\textsf{burn-verifier}\text{ extends }\textsf{crosschain}$_{k,m,z}$; \textsf{ERC20}}
            \State\textsf{mapping}(\textsf{address} $\Rightarrow$ \textsf{uint256}) \textsf{balances}
            \State{$\textsf{claimed-events} \gets \emptyset$}
            \Function{\sf is-claimable-event}{$e$}
                \State\Return{$e \notin \textsf{claimed-events} \land \textsf{event-exists}(e)$}
            \EndFunction
            \Function{\sf claim}{$e$}
                \If{$\lnot \textsf{is-claimable-event}(e)
                    \lor \lnot \BurnVerify(\textsf{msg.sender}, e.\textsf{addr})$}
                    \State\Return{$\bot$}
                \EndIf

                \Let{\textsf{claimed-events}}{\textsf{claimed-events} \cup \{e\}}
                \State{$\textsf{balances[msg.sender]} \pluseq e.\textsf{amount}$}
            \EndFunction
        \EndContract
        \vskip8pt
    \end{algorithmic}
\end{algorithm}
