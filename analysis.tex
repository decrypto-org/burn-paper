...
\begin{proof}
  ...
  by $\textsf{Gen}$ generated with some probability $p$.
  Then consider the existential forgery adversary $\mathcal{A}$ which works as
  follows. It receives $pk$ as its input from the challenger, but ignores it
  and generates a new key pair $(pk', sk') \gets \textsf{Gen}$. Since the
  ...
  $\Pr[\textsf{Sig-forge}^{cma}_{\mathcal{A},S}] = negl(\kappa)$.
  But $\Pr[pk = pk'] \leq \Pr[\textsf{Sig-forge}^{cma}_{\mathcal{A},S}]$ and
  therefore $p = negl(\kappa)$. Applying Lemma~\ref{lem:negl-pred}, we deduce
  that the distribution $X$ is unpredictable.
\end{proof}
...
\begin{proof}
  ...
  $\Pr[\textsc{predict}_{\mathcal{A}^*,\mathcal{T}}(\kappa) = \true]
   ...
   = \frac{1}{r}\Pr[\query]$.
  As
  $\Pr[\textsc{predict}_{\mathcal{A}^*,\mathcal{T}}(\kappa) = \true] \leq negl(\kappa)$
  and $r$
  is polynomial in $\kappa$, we deduce that $\Pr[\query] \leq negl(\kappa)$.
