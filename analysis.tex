...
\begin{theorem}[Unspendability]
  If $H$ is preimage-resistant, then the protocol $\Pi$ of Section~\ref{section:construction} is \emph{unspendable}.
\end{theorem}
\begin{proof}
  ...
  This would imply that when successful, $\mathcal{A}$ can generate $t$, $m$, $\sigma = (pk, \sigma')$ and $pkh$ such that $\BurnVerify(1^\kappa, t, pkh) = \true \land \SpendVerify(m, \sigma, pkh) = \true$.

  From the definition of $\BurnVerify$ we know that $pkh = H_\kappa(t) \xor 1$.
