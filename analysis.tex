...
\begin{proof}
  ...
  If $pk = pk'$ then by the correctness of the signature scheme the forgery will be successful. The forgery challenger $\textsf{Sig-forge}$ and the adversary $\mathcal{A}$ each independently generates a pair of keys. By applying a union bound we obtain that $p^2 \Pr[\textsf{Sig-forge}^{cma}_{\mathcal{A},S}] \geq \Pr[\textsc{repeat-key}]$. From the fact that $p$ is a polynomial and given that
