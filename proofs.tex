...
Applying an efficiently computable function to
...
Public keys generated from secure signature schemes must
be unpredictable.
...
\begin{proof}
  ...
  therefore $p \leq \sqrt{\Pr[pk = pk']} \leq \negl$. From this, we deduce that the distribution ensemble $X_\kappa$ is
  unpredictable.
\end{proof}
...
\begin{proof}
  ...
  indistinguishability game in which the distinguisher gives a guess $b^*$
  attempting to identify the origin $b$ of its input
  (with $b = 0$ indicating that the first distribution was sampled,
   and $b = 1$ indicating that the second distribution was sampled).
