...
For a verifier to be convinced that an event has taken place on a source blockchain, they need to ensure that its transaction is contained the best source chain. Specifically, the following data needs to be supplied to the smart contract as a proof:
...
Having a valid $\mathcal{M}$, a verifier verifies a Merkle Tree inclusion proof \textsf{blockConnection} for $\textsf{b} \in \mathcal{M}$ and is certain the block provided is part of the best source chain. This approach is illustrated in Algorithm~\ref{alg.is-in-best-chain-federation}.
